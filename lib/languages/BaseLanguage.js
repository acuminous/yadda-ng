...
module.exports = class Language {
  constructor({ name = this.constructor.name, vocabulary }) {
    ...
    this._regexp = {
      ...
      feature: new RegExp(`^(?:${vocabulary.feature.join('|')})\\s*:\\s*(.*)`, 'i'),
      background: new RegExp(`^(?:${vocabulary.background.join('|')})\\s*:\\s*(.*)`, 'i'),
      scenario: new RegExp(`^(?:${vocabulary.scenario.join('|')})\\s*:\\s*(.*)`, 'i'),
    };
  }
