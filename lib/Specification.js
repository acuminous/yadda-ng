...
module.exports = class Specification {
  ...
  constructor(props = {}) {
    this._language = props.language || Languages.instances.default;
  ...
  _parseLine(line, number) {
    [
      this._createHandler('language', /^\s*#\s*language\s*=(.*)$/, (match) => ({ name: match[1].trim() })),
  ...
  setLanguage({ name }) {
    this._language = Languages.find(name);
  }
