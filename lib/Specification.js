...
module.exports = class Specification {
  ...
  _parseLine(line, number) {
    ...
    [
      ...
      this._createHandler('feature', language.regexp('feature'), (match) => ({ title: match[1].trim() })),
      this._createHandler('background', language.regexp('background'), (match) => ({ title: match[1].trim() })),
      this._createHandler('scenario', language.regexp('scenario'), (match) => ({ title: match[1].trim() })),
      ...
      this._createHandler('step', language.regexp('step'), (match) => ({ statement: match[0].trim(), generalised: match[1].trim() })),
