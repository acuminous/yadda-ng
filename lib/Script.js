...
module.exports = class Script {
  ...
  _compileScenario(librarian, scenario) {
    ...
    const steps = scenario.steps.map((step) => this._compileStep(competition, libraries, step));
    return new Scenario({ annotations, title, steps });
  }

  _compileStep(competition, librarian, step) {
    const annotations = this._compileAnnotations(step.annotations);
    const text = step.text;
    return librarian.createStep(competition, annotations, text);
  }
