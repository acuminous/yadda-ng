...
module.exports = class DynamicStep extends BaseStep {
  ...
  constructor({ librarian, annotations, text, generalised }) {
    super({ annotations, text, generalised });
  ...
  async run(state) {
    ...
    const step = this._createStep(this._annotations, this.text, this.generalised, ranked);
  ...
  _createStep(annotations, text, generalised, ranked) {
    if (ranked.winner) return new RunnableStep({ annotations, text, generalised, macro: ranked.winner });
    if (ranked.contenders) return new AmbiguousStep({ text, generalised, contenders: ranked.contenders });
    return new UndefinedStep({ annotations, text, generalised });
  }
};
