const debug = require('debug')('yadda:Library');
...
// The job of a Library is to resolve steps to macros
module.exports = class Library {

  constructor(props = {}) {
    this._name = props.name || this._randomise(this.constructor.name);
    this._dictionary = Dictionary.combine(props.dictionaries || []);
    this._macros = [];
  }
  ...
  define(signatures, fn, options = {}) {
    ...
    [].concat(signatures).forEach((signature) => {
      const macro = signature.source
        ? this._createMacroFromRegExp(signature, fn, options)
        : this._createMacroFromTemplate(signature, fn, options);
