...
module.exports = class Librarian {
  ...
  _makeStep(annotations, text, ranked) {
    ...
    if (ranked.winner && ranked.winner.isPending()) return new PendingStep({ text, macro: ranked.winner });
    if (ranked.winner) return new AsyncStep({ annotations, text, macro: ranked.winner });
