...
module.exports = class SpecificationParser {
  ...
  static _parseLines(text, session) {
    text.split(/\n|\r\n/).concat('\u0000').forEach((line, index) => SpecificationParser._parseLine(session, line, index + 1));
  }

  static _parseLine(session, line, number) {
    debug(`Parsing line ${number}: "${line}" in state: ${session.machine.state}`);
    const event = SpecificationParser._getEvents(session).find((event) => event.test(line));
