...
module.exports = class StateMachine {
  ...
  constructor(props = {}) {
    ...
    this._events = [ EndEvent, DocstringEvent, LanguageEvent, MultiLineCommentEvent, SingleLineCommentEvent, AnnotationEvent, FeatureEvent, BackgroundEvent, ScenarioEvent, BlankLineEvent, StepEvent, TextEvent ];
  }
  ...
  handle(source, session) {
    this._events.map((clazz) => new clazz()).find((event) => event.handle(source, session, this._getCurrentState()));
  }
