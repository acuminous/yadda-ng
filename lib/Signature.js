...
module.exports = class Signature {
  constructor(props) {
    const { library, template, pattern, debug = Debug('yadda:Signature') } = props;
  ...
  // TODO rename to something that doesnt suggest knowledge of what the signature is doing with the state
  // e.g. prepare(state)
  ...
  toString() {
    return this._template ? `signature [${this._pattern}] derived from template [${this._template}] defined in library [${this._library.name}]` : `signature [${this._pattern}] defined in library [${this._library.name}]`;
  }
