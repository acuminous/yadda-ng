...
module.exports = class Dictionary {
  ...
  static combine(dictionaries) {
    return [].concat(dictionaries).reduce((master, dictionary) => {
      return dictionary.copyInto(master);
    }, new Dictionary());
  }

  copyInto(other) {
    return this._terms.reduce((dictionary, term) => {
      return term.defineIn(other);
    }, other);
  }
  ...
  expand(template) {
    ...
    const { source } = this._expand(template);
    const pattern = this._createPattern(`^${source}$`, template);
    return { pattern, converter: new PassthroughConverter() };
  }

  _createPattern(source, template) {
  ...
  _expand(template, history = []) {
    ...
    }, { template, mode: 'text', delimited: false, expression: '', source: '', history });
  }
  ...
  _parseTerm(state, letter) {
    const { delimited, expression, source, history } = state;
    if (expression === '' && letter === null) return { ...state, delimited: false, expression: '', source: `${source}${this._prefix}`};
    if (letter === null) return { ...state, delimited: false, expression: '', source: `${source}${this._resolve(expression, history).source}`};
    ...
    if (letter === this._delimiter) return { ...state, delimited: true };
    if (letter === this._prefix) return { ...state, expression: '', source: `${source}${this._resolve(expression, history).source}` };
    ...
    return { ...state, mode: 'text', expression: '', source: `${source}${this._resolve(expression, history).source}${letter}` };
  }
  ...
  _resolve(expression, history) {
    ...
    return this._expand(term.definition, history.concat(expression));
  }
