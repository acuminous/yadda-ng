...
module.exports = class Dictionary {
  ...
  _find(expression, fallback) {
    return this._terms.find((term) => term.resolves(expression)) || fallback;
  }
  ...
  _resolve(expression, history) {
    ...
    const term = this._find(expression, new Term({ expression, pattern: /(.+)/, converters: [ new PassthroughConverter() ]}));
    return this._parseTemplate(term.definition, history.concat(expression));
  }
