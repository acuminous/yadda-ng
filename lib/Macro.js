...
module.exports = class Macro {
  ...
  async run(state, statement) {
    const args = await this._parseArguments(state, statement);
  ...
  _match(statement) {
    ...
    return this._pattern.exec(statement);
  }

  async _parseArguments(state, statement) {
    const matches = this._match(statement).slice(1);
