...
// TODO uncomfortable making _definition a string
module.exports = class Term {
  constructor(props) {
    const { expression, definition = /(\S+)/, converters, debug = Debug('yadda:Term') } = props;
  ...
  // TODO Odd that need to temporarily use MultiConverter to get the demand
  _checkConverterArguments() {
    ...
    if (supply !== demand) {
      throw new Error(
        `Pattern [${this._definition}] for term [${this._expression}] ${supply < demand ? 'has only' : 'has'} ${supply} matching ${supply === 1 ? 'group' : 'groups'}, ${supply < demand ? 'but' : 'but only'} a total of ${demand} converter ${demand === 1 ? 'argument was' : 'arguments were'} specified`
      );
    }
  }
