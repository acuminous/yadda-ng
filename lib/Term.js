...
module.exports = class Term {
  ...
  _checkConverterArguments() {
    ...
    if (numMatchingGroups > 0 && numConverters === 0) {
      throw new Error(`Pattern [${this._definition}] for term [${this._expression}] has ${numMatchingGroups} matching ${numMatchingGroups === 1 ? 'group' : 'groups'}, but no converters were specified`);
    }
    if (numMatchingGroups !== numConverterArguments) {
      throw new Error(`Pattern [${this._definition}] for term [${this._expression}] ${numMatchingGroups < numConverterArguments ? 'has only' : 'has'} ${numMatchingGroups} matching ${numMatchingGroups === 1 ? 'group' : 'groups'}, ${numMatchingGroups < numConverterArguments ? 'but' : 'but only'} ${numConverters} ${numConverters === 1 ? 'converter' : 'converters'} with a total of ${numConverterArguments} ${numConverterArguments === 1 ? 'argument was' : 'arguments were'} specified`);
    }
  }
