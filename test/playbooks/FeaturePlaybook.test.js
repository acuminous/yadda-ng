...
describe.only('Feature Playbook', () => {
  ...
  it('should pretend to run features', async () => {
    ...
    expect(report[0].feature).toBe(feature1);
    expect(report[8].feature).toBe(feature2);

    expect(report[0].scenario).toBe(scenario1);
    expect(report[4].scenario).toBe(scenario2);
    expect(report[8].scenario).toBe(scenario1);
    expect(report[12].scenario).toBe(scenario2);

    expect(report[0].step).toBe(step1);
    expect(report[0].status).toEqual('run');
    expect(report[1].step).toBe(step2);
    expect(report[1].status).toEqual('undefined');
    expect(report[2].step).toBe(step3);
    expect(report[2].status).toEqual('pending');
    expect(report[3].step).toBe(step4);
  ...
  it('should run features', async () => {
    ...
    const step1 = new AsyncStep({ statement: 'one', macro });
    const step2 = new PendingStep({ statement: 'two' });
    ...
    expect(report[0].feature).toBe(feature1);
    expect(report[4].feature).toBe(feature2);

    expect(report[0].scenario).toBe(scenario1);
    expect(report[2].scenario).toBe(scenario2);
    expect(report[4].scenario).toBe(scenario1);
    expect(report[7].scenario).toBe(scenario2);
