...
const { Macro, Converters } = require('..');
...
describe('Macro', () => {
  ...
  it('should run step functions', async () => {
    const spy = new Spy();
    const macro = new Macro({ pattern: /.*/, converters: [], fn: spy.tap() });
  ...
  it('should allow state changes', async () => {
    const state = {};
    const macro = new Macro({ pattern: /.*/, converters: [], fn: (state) => state.updated = true });
    await macro.run(state, 'Some Text');
  ...
  it('should pass parsed arguments to step functions', async () => {
    ...
    const spy = new Spy();
    const macro = new Macro({ pattern: /^(.+) (.+)$/, converters, fn: spy.tap(3) });
  ...
  it('should raise an error when there are fewer matches than converter arguments', async () => {
    ...
    const spy = new Spy();
    const macro = new Macro({ pattern: /^(.+)$/, template: '$name', converters, fn: spy.tap() });
  ...
  it('should raise an error when there are fewer parsed arguments than step arguments', async () => {
    const converters = [new PassthroughConverter()];
    const macro = new Macro({ pattern: /^(.+)$/, template: '$name', converters, fn: (state, a, b) => {}});
  ...
  it('should raise an error when there are more parsed arguments than step arguments', async () => {
    ...
    const macro = new Macro({ pattern: /^(.+) (.+)$/, template: '$name', converters, fn: (state, a) => {}});
